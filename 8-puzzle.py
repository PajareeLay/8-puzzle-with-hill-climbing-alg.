...
from simpleai.search import astar, SearchProblem, hill_climbing
import random
...
class EigthPuzzleProblem2(SearchProblem):
    def actions(self, state):
        '''Returns a list of the pieces we can move to the empty space.'''
        rows = string_to_list(state)
        row_e, col_e = find_location(rows, 'e')
    ...
    def diff_distance(value, state):
        rows = string_to_list(state)
        collect_distance.append(value)
        collect_state.append(rows)

        max_value = max(collect_distance)
        max_position = collect_distance.index(max_value)
        best_state = collect_state[max_position]
...
result2 = hill_climbing(EigthPuzzleProblem2(INITIAL))
collect_distance = []
collect_state = []
print('Move number', result2.action)
print(result2.state)

'''while result2.state != GOAL:
    print('Move number', result2.action)
    print(result2.state)
'''